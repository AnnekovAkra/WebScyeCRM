...
(function() {
    ...
    // 1. Функция экспорта (Привязка к кнопке Экспорт)
document.querySelector('button:contains("Экспорт")').addEventListener('click', () => {
    window.location.href = 'export.php'; // Просто перенаправляем на скрипт, и файл скачивается
});

// Продвинутый селектор, если класс кнопки известен, замени на свой класс:
// document.querySelector('.export-btn-class').onclick = ...

// 2. Функция отправки нового пользователя из интерфейса (Пример вызова)
async function addUser(name, email, role, status) {
    ...
    if (response.ok) {
        ...
        console.log('Пользователь добавлен в flat-file базу:', result);
        // Здесь можно вызвать функцию обновления таблицы на экране
    }
}
